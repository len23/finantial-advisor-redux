...
import { setinInitialValues, setCurrentValue, selectCurrentValues, selectRecomendation, setNewValues } from './recomendationsSlice';
...
export default function Recomendations() {
  ...
  const makeCalculations = () => {
    const total = Object.values(currentValues).reduce((pv, cv) => parseInt(pv) + parseInt(cv));
    let portTemp = {...portfolioValues}
    for (const key in portfolioValues) {
      let newAmount = (total * selectedRisk[key] / 100).toFixed(2);
      let difference = (newAmount - parseInt(portfolioValues[key].current)).toFixed(2);
      portTemp[key] = {...portTemp[key], difference: difference, newAmount: newAmount};
    }
  ...
  return (
    <div>
      ...
      <Row>
        <Col lg={5}>
          <CurrentPortfolio onSubmit={handleSubmit} onChange={handleChange} labels={currentValues} />
        </Col>
        <Col lg={7}>
          <TableResults />
        </Col>
