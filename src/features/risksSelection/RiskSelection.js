...
export default function RiskSelection() {
  ...
  const handleDataDisplay = () => {
    setShowGraphic({
      show: !showGraphic.show,
      srcImg: showGraphic.show ? pie : list,
    });
  };
  ...
  return (
    <div>
      <SwitchButtton
        ...
        src={showGraphic.srcImg}
      />
      <RiskLevelsSelectors
        riskLevels={riskLevels}
        onContinue={handleContinue}
      />
      {!showGraphic.show && <TableLevels risksDataDetails={risksDataDetails} />}
      {showGraphic.show && (
        <DoughnutChart riskSelectedDetails={riskSelectedDetails} />
      )}
    </div>
  );
}
